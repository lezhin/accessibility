...
성공(또는 결과) 상태 메시지를 사용자에게 전달하는 콘텐츠입니다. 성공 `role="status"` 메시지가 알럿 `role="alert"`과 다른 것은 키보드 초점을 임의로 변경하지 않고(사용자의 현재 작업을 방해하지 않고) 보조기기 사용자에게 메시지를 전달하는 것을 의도하고 있다는 점입니다.
